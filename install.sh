...
# 2. PEDIR DATOS AL USUARIO (SOLO TOKEN E ID)
echo -e "\n${CYAN}📝 PASO 1: CREDENCIALES${NC}"
echo -e "${YELLOW}🔑 Ingresa los datos para configurar tu bot:${NC}"
...
if [ -z "$BOT_TOKEN" ] || [ -z "$ADMIN_ID" ]; then
    echo -e "${RED}❌ Error: No ingresaste el Token o el ID. Cancelando instalación.${NC}"
    exit 1
fi
...
# 8. CREAR EL SERVICIO DE SYSTEMD
